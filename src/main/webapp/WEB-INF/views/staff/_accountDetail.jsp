...
<div class="main-content flex-1 px-5 py-3 bg-[#F5F4FA] overflow-y-auto max-h-full overflow-visible">
    <div class="max-w-7xl mx-auto flex flex-col h-full">
        ...
        <c:choose>
        <c:when test="${account.accountType == 'user'}">
            ...
            <c:otherwise><div id="result-container" class="mt-4"></div></c:otherwise>
        </c:choose>
    </div>
</div>
<script>
    ...
    function loadResults() {
        ...
        const staffId = "${account.accountId}";

        // Nếu không có staffId => bỏ qua
        if (!staffId) {
            console.warn("No staffId provided");
            return;
        }

        // Hiển thị loading
        document.getElementById("result-container").innerHTML =
            "<p class='text-gray-500'>Loading staff info...</p>";

        fetch(`${contextPath}/staff?staffId=${staffId}`, {
            method: 'GET',
    ...
    // ✅ Chỉ chạy khi là staff
    <c:if test="${account.accountType eq 'staff'}">
