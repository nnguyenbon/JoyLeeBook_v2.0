...
-- ============================================
IF EXISTS (SELECT name FROM sys.databases WHERE name = N'JoyLeeBook')
BEGIN
...
CREATE TABLE review_chapter (
    ...
    CONSTRAINT FK_review_series_chapter FOREIGN KEY (chapter_id) REFERENCES chapters(chapter_id) ON DELETE CASCADE,
    CONSTRAINT FK_review_series_staff FOREIGN KEY (staff_id) REFERENCES staffs(staff_id)
);
GO

CREATE INDEX IX_review_series_staff ON review_series(staff_id, updated_at DESC);
CREATE INDEX IX_review_series_status ON review_series(status);
GO
